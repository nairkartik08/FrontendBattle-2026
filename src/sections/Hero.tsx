...
export function Hero() {
  return (
    <section id="top" className="relative overflow-hidden bg-arctic pt-20" aria-labelledby="hero-title">
      <section className="hero-grid relative mx-auto grid min-h-[calc(100vh-5rem)] w-full max-w-7xl items-center gap-12 px-4 py-14 sm:px-6 lg:grid-cols-[0.95fr_1.05fr] lg:px-8 lg:py-20">
        <div className="relative z-10">
          <div className="inline-flex items-center gap-2 rounded-[8px] border border-nocturnal/15 bg-white/55 px-3 py-2 text-sm font-bold text-nocturnal shadow-sm backdrop-blur">
            <Icon name="arrowTrendingUp" className="h-4 w-4" />
          ...
          <dl className="mt-10 grid max-w-xl grid-cols-3 gap-3">
            {metrics.map(([value, label]) => (
              <div key={label} className="rounded-[8px] border border-nocturnal/10 bg-white/45 p-3 backdrop-blur">
                <dt className="font-mono text-xl font-bold text-oceanic">{value}</dt>
        ...
        <div className="relative z-10 mx-auto w-full max-w-2xl">
          <div className="absolute -left-6 top-8 hidden h-16 w-16 animate-float rounded-[8px] bg-forsythia/95 p-4 shadow-glow md:block">
            <Icon name="chartPie" className="h-full w-full" />
          </div>
          <div className="absolute -right-3 bottom-20 hidden h-14 w-14 animate-float-delayed rounded-[8px] bg-saffron p-3 shadow-panel md:block">
            ...
            <div className="rounded-[8px] bg-oceanic p-4 text-arctic">
              <div className="flex items-center justify-between border-b border-white/10 pb-4">
                ...
                <span className="grid h-11 w-11 place-items-center rounded-[8px] bg-white/10">
                  <Icon name="arrowPath" className="h-5 w-5 invert" />
                </span>
              </div>
              ...
              <div className="mt-5 grid gap-4 lg:grid-cols-[1fr_0.82fr]">
                <div className="rounded-[8px] bg-white/8 p-4">
                  <div className="flex items-end gap-2">
                    {[54, 72, 46, 82, 66, 90, 78].map((height, index) => (
                      <span
                        key={height + index}
                        className="hero-bar block flex-1 rounded-t-[6px] bg-gradient-to-t from-saffron to-forsythia"
                ...
                <div className="space-y-3">
                  {activity.map(([title, detail, colorClass]) => (
                    <div key={title} className="rounded-[8px] bg-white/10 p-3">
        ...
        <a
          href="#features"
          className="absolute bottom-5 left-1/2 hidden -translate-x-1/2 items-center gap-2 font-mono text-xs font-bold uppercase tracking-[0.2em] text-nocturnal/70 lg:flex"
        >
          Scroll
